                ...
                if (s == 1.0) {
                    if (configuration.autoflower) {
                        ...
                        if (itm != null && itm.selected && !ui.modctrl && (!ignoreAutoSetting || p.name.equals("Peer into")) ||
                                p.name.equals(nextAutoSel) || p.num == nextAutoSelI || (nextAutoSelb && opts.length == (CLOSEFORMENU.get() ? 2 : 1)) && System.currentTimeMillis() - nextAutoSelTimeout < 2000) {
                            nextAutoSel = null;
                            nextAutoSelI = -1;
                            nextAutoSelb = false;
//                        try {
...
//                        }
    ...
    public void choose(Petal option) {
        if (ui.gui != null && ui.gui.petalClickCallback != null) {
...
//                ui.gui.map.pllastcc = null;
    ...
    public static void setNextSelection(int index) {
        nextAutoSelI = index;
        nextAutoSelTimeout = System.currentTimeMillis();
    }
}
